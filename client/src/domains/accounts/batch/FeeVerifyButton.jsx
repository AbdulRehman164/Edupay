...
function FeeVerifyButton({ s, setStudents }) {
    const { id } = useParams();
    async function handleSubmit(s, action) {
        const res = await fetch(
            `/api/accounts/batches/${id}/students/verify/${s.id}?action=${action}`,
            { method: 'PATCH' },
        );
        if (res.ok) {
            setStudents((prev) =>
                prev.map((student) =>
                    student.id === s.id
                        ? {
                              ...student,
                              fee_verified: action === 'true' ? true : false,
                          }
                        : student,
                ),
            );
...
export default FeeVerifyButton;
